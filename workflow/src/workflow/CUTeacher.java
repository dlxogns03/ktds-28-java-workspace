...
public class CUTeacher {
	public static int pay(int amount, int cash,int CARD_LIMIT, int cardPaidMoney,
	...
	public static int pointAdder(int amount) {
		int earnPoint = (int)(amount * 0.1f);
		return earnPoint;
	}
	
	public static int buyItems(int[] iteams, float[] dc) {
		int amount = 0;
		for (int i = 0; i < iteams.length; i++) {
			int itemPrice = (int) (iteams[i] * (1 - dc[i]));
			amount += itemPrice;
		}
	...
	public static void main(String[] args) {
		...
		// 포인트 적립 (0 ==> 적립, 1 ==> 적립 안함)
		int addPoint = (int) (Math.random() * 02);
		// 적립을 할 경우 (addPoint == 0)이라면 총 결제 금액에서 10% 적립.
		if (addPoint == 0) {
			int earnPoint = pointAdder(amount);
