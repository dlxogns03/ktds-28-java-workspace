...
public class ExceptionTest {
	public static void main(String[] args) {
		...
		String name = "asdf";
		// name이라는 인스턴스의 값이 null이라면 "이름을 입력해주세요를 출력한다.
		...
		//아니라면 값을 출력한다.   job.trim().length()==0도 들어갈수 있다.
		...
		String address= "    ";
		//address라는 인스턴스의값이 공백으로만 이루어져 있다면 " 주소를 입력해주세요를 출력한다"
		//아니라면 값을 출력한다
		String message = StringUtils.emptyToDefault(address, "주소를 입력해주세요");
		System.out.println(message);
		address.replaceAll("  ","");
		if(address == null || address.isBlank()) {
			System.out.println("주소를입력해주세요 ");
		}else {
