...
public class DateTime {
	...
	public static void setAndPrintDateTime(int year, int month, int day,
											int hour, int minute, int second) {
		LocalDateTime set = LocalDateTime.of(year, month, month, hour, minute, second);
	...
	public static void setAndPrintTime(String date) {
		LocalTime alarm = LocalTime.parse(date);
	
		System.out.println(alarm);
	}
	
	public static void setAndPrintDateTime(String date) {
		LocalDateTime set = LocalDateTime.parse(date);
		System.out.println(set);
	}
	...
	public static void printNowTimeWithFormat() {
		...
		//s:초
		LocalTime now = LocalTime.now();
		DateTimeFormatter format = DateTimeFormatter.ofPattern("H시 ");
	...
	public static void printPeriodFromTo(LocalDate from, LocalDate to) {
		...
		System.out.println("%s 부터 %s 까지 %d년 %d개월 %d일 지났습니다.".formatted(from, to,
	...
	public static void printIsAfter(LocalDate date) {
		...
		LocalDate now = LocalDate.now();
		System.out.println("%s는 %s보다".formatted(now,date));
		
		boolean isPast = now.isAfter(date);
...
//		setAndPrintTime(25,30,11);//java.time.DateTimeException
		
		
		setAndPrintDateTime(2026, 8, 14, 12, 30, 30);
		
	
		setAndPrintDate("2026-02-18");
		
		
		setAndPrintTime("11:03:11");
		
		setAndPrintDateTime("2026-04-01T12:30:14");
...
//		setAndPrintTimeWithFormat("11:11:11", "hh시mm분ss초");
//		setAndPrintDateTimeWithFormat("2026-08-24T14:14:43", "yyyy년MM월dd일 hh시mm분ss초");
		plusYearsAndPrintDateFromNow();
		plusDaysAndprintDateFromNow();
		
		
		printPeriodFromTo(LocalDate.parse("2003-03-12"), LocalDate.now());
		
		printBetweenDaysFromTo(LocalDate.parse("2003-03-12"),LocalDate.now());
		
		printIsAfter(LocalDate.parse("2026-08-03"));
		printIsAfter(LocalDate.parse("2026-08-05"));
		printIsAfter(LocalDate.parse("2026-08-11"));
		
		printIsBefore(LocalDate.parse("2026-08-11"));
